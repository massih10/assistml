...
model_bank = H2ODeepLearningEstimator(hidden = [10,10,10,10,10],balance_classes = True,epochs = 10,nfolds = 5, fold_assignment = 'Stratified',l1 = 0.01, mini_batch_size = 100, input_dropout_ratio = 0.1, seed = 25, standardize = True)
start=time.time()
model_bank.train(x=x,y=y, training_frame=data_train_all, model_id="DLN_kick_015")
end=time.time()
t=(end-start)    
...
import os,json
if os.path.exists('/home/sruthi/asm-2/asm-2/3_pickle/DLN_kick_015.json'):
    with open('/home/sruthi/asm-2/asm-2/3_pickle/DLN_kick_015.json', 'r') as f:
        models = json.load(f)
    models["Metrics"] = metrics
    with open('/home/sruthi/asm-2/asm-2/3_pickle/DLN_kick_015.json', 'w') as f:
