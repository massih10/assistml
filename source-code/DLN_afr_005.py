...
model_review = H2ODeepLearningEstimator(hidden = [10,10,10],balance_classes = True,epochs = 10,nfolds = 5, fold_assignment = 'Stratified',l1 = 0.01, mini_batch_size = 100, input_dropout_ratio = 0.1, seed = 50, standardize = True)
start=time.time()
model_review.train(x=x,y=y, training_frame=train, model_id="DLN_afr_005")
end=time.time()
t=(end-start)
...
import os,json
if os.path.exists('/home/sruthi/asm-2/asm-2/3_pickle/DLN_afr_005.json'):
    with open('/home/sruthi/asm-2/asm-2/3_pickle/DLN_afr_005.json', 'r') as f:
        models = json.load(f)
    models["Metrics"] = metrics
    with open('/home/sruthi/asm-2/asm-2/3_pickle/DLN_afr_005.json', 'w') as f:
