...
class BuildMLModel:
    ...
    def store_train_test_dataset_in_csv(self, train_csv_name, test_csv_name):
        ...
        y_test_df.rename(columns={0: "Class"}, inplace=True)
        ...
        test_df.to_csv(test_csv_name, index=False, header=True)
    ...
    def data_pre_processing(self):
        ...
        for index, feature in enumerate(categorical_feature_names):
            ...
            X_train_categorical.iloc[:, index] = col_le.fit_transform(X_train_categorical.iloc[:, index])
            X_test_categorical.iloc[:, index] = col_le.transform(X_test_categorical.iloc[:, index])
    ...
    def create_SVM_classifier(self):
        """
        Creates and returns a SVM Linear Kernel Classifier
        :return: clf: SVM Linear Kernel Classifier
        """
        self.clf = create_model('svm')
        return self.clf

    def tune_model(self, params=None):
        tune_start_time = time.time()
        if params is not None:
            self.clf = tune_model(self.clf, custom_grid=params)
        else:
            self.clf = tune_model(self.clf)
...
if __name__ == "__main__":
    ...
    train_csv_name = "bank_train_" + str(int(train_size * 100)) + ".csv"
    train_csv_path = os.path.join(data_base_path, train_csv_name)
    test_csv_name = "bank_test_" + str(int((1 - train_size) * 100)) + "_" + str(seed_value)+".csv"
    ...
    algorithm_implementation = "pycaret.classification.create_model('svm')"

    pycaret_setup_obj = build_ml_model_obj.setup_pycaret(seed_value)
    SVM_clf = build_ml_model_obj.create_SVM_classifier()
    logger.info("Default SVM - Linear Kernel model {}".format(SVM_clf))

    alpha_list = [0.01, 0.02, 0.05, 0.08, 0.1, 0.15, 0.2, 0.25, 0.3]

    # find out a good alpha hyperparameter
    params = {'alpha': alpha_list}
    tuned_SVM_clf, tune_duration = build_ml_model_obj.tune_model(params=params)
    logger.info("After tuning SVM - Linear Kernel model {}".format(tuned_SVM_clf))

    hyper_parameters_dict = {}
    grid_search_dict = {}
    grid_search_dict["alpha"] = [str(a) for a in alpha_list]
    hyper_parameters_dict["Grid_Search"] = grid_search_dict

    alpha = 0.2
    hyper_parameters_dict["alpha"] = str(alpha)
    hyper_parameters_dict["eta0"] = str(0.001)
    hyper_parameters_dict["random_state"] = str(seed_value)
    ...
    final_SVM_clf, train_duration = build_ml_model_obj.fit_with_classifier()

    # store the model into pickle file
    pickle_file_name = "SVM_bank_007"  # No need to put .pkl extension. pycaret save model automatically puts .pkl extension
    ...
    json_file_name = "SVM_bank_007.json"
